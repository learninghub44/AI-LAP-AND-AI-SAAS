...
Docker Compose is the recommended way to run Cotell AI for personal use. The container serves the Express API and the built React dashboard from one process on port 3001, with SQLite persisted in a named volume.
...
Open http://localhost:3001, add provider keys on the **Keys** page, then use the generated `cotell-...` key with any OpenAI-compatible client.
...
curl http://localhost:3001/v1/chat/completions \
  ...
  -d '{
    "model": "auto",
    "messages": [{"role": "user", "content": "Say hello from Cotell AI."}]
  }'
```
...
The `cotell-data` volume stores SQLite data at `/app/server/data`. Keep the same volume and `ENCRYPTION_KEY` when upgrading, otherwise existing encrypted provider keys cannot be decrypted.
...
```bash
docker pull ghcr.io/tashfeenahmed/cotell:latest
```
