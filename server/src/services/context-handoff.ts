import type { ChatMessage } from '@cotell/shared/types.js';
...
export function getContextHandoffMode(): ContextHandoffMode {
  const raw = process.env.COTELL_CONTEXT_HANDOFF?.trim().toLowerCase();
...
export function maybeInjectContextHandoff(params: {
  ...
  const alreadyPresent = messages.some(m => {
    ...
    return text.startsWith('Cotell AI context handoff:');
  });
